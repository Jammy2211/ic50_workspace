"""
Graphical-model fit on real GDSC2 IC50 data.

Companion to `ep_real.py` — same factor graph, single non-linear search
over the joint parameter space instead of EP message passing. See
`graphical_sim.py` for the discussion of how graphical and EP fits
relate and why both are kept in the workspace.

Loads the first 5 datasets from `dataset/real/drug_<id>/` and fits the
global linear + per-dataset Hill factor graph as one joint model.
There is no ground truth so no recovery assertion fires — this script
just confirms the graphical-fit path runs end-to-end on real data and
...
Usage:
    python3 scripts/graphical_real.py                # fits BOTH drugs sequentially
    python3 scripts/graphical_real.py --drug_id 1003
...
def fit_drug(drug_id):
    ...
    plot_dir = sample_path / "graphical_results"
    plot_dir.mkdir(parents=True, exist_ok=True)

    name = f"real_{drug_id}"
    print(f"\n=== ic50_workspace graphical fit ({name}) ===")
...
def main():
    ...
    if is_test_mode():
        ...
        print("\nGraphical fit on real data complete.")
